...
1. find out the longest number in the column, and add trailing 0s to make shorter numbers equivalent in length.
...
def format_numbers(grid, index):
    ...
    ## find longest and add trailing 0s
    longest = max(numbers, key=len)
    for number in numbers:
        if len(number) < len(longest):
            number += '0' * (len(longest) - len(number))
